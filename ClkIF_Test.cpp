...
int main (){
	...
	mytest = CNT_IF(0xAAAA111133334444, 0xFFFF000000000000,0,1,1,0xAAAACCCCBBBBFFFF);
	goldinput(0,1,1,0xAAAACCCCBBBBFFFF,1);
	comp(mytest, gold);


	mytest = CNT_IF(0xAAAA111133334444, 0x55550000FFFF0000,0,0,0,32);
	goldinput(0,0,0,0,0);
	comp(mytest, gold);

	mytest = CNT_IF(0xAAAA1111BBBB4444, 0x0000000000000000, 1,0,1,0xA321C58CBBBBFFFF);
	goldinput(1,0,1,0xA321C58CBBBBFFFF,1);
	comp(mytest, gold);

	mytest = CNT_IF(0xAAAA1111BBBB0001, 0x000000000000FFFF, 1,0,1,0xA321C58CBBBB0011);
	goldinput(1,0,1,0xA321C58CBBBB0011,0);
	comp(mytest, gold);
